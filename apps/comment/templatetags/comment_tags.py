...
from ..models import ArticleComment,AboutComment,CommentUser,MessageComment,Comment
import collections
...
# @register.simple_tag
# def get_comment_childrens(category,entry=0):
#     '''展示父评论下所有子评论'''
#     if category=='about':
#         lis=AboutComment.objects.all()
#     elif category=='message':
...
#         lis = ArticleComment.objects.filter(belong_id=entry)
    ...
    #     lis_dict = {}
    #     lis_dict['id']=li.id
    #     lis_dict['w']=li.content
    #     lis_dict['parent']=li.parent
    #     lis_dict['children']=[]
    #     comment.append(lis_dict)
    # comment_dict={}
    # for k,v in enumerate(comment):
    #     comment_dict[v['id']]=v
    # ret=[]
    # for k in comment_dict:
    #     if comment_dict[k]['parent'] is not None:
    #         comment_dict[comment_dict[k]['parent']]['children'].append(comment_dict[k])
    #     else:
    #         ret.append(comment_dict[k])
